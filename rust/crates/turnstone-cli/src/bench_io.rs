...
//! Bench hot-path I/O matching Go `bench.go`: write on the TLS socket, read via `BufReader`.
...
impl BenchConn {
    pub fn connect(opts: &ConnectOptions) -> Result<Self, String> {
        ...
        let conn = ClientConnection::new(tls, server_name).map_err(|e| e.to_string())?;
