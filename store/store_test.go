...
func TestStore_Recover_Basic(t *testing.T) {
	...
	s1, err := NewStore(dir, logger, true, 0, false, "time")
	if err != nil {
	...
	s2, err := NewStore(dir, logger, true, 0, false, "time")
	if err != nil {
...
func TestStore_Replication_Quorum(t *testing.T) {
	...
	s, err := NewStore(dir, logger, true, 1, false, "time")
	if err != nil {
		t.Fatal(err)
	}
	defer s.Close()
...
func TestStore_Replication_ApplyBatch(t *testing.T) {
	...
	s, err := NewStore(dir, logger, true, 0, false, "time")
	if err != nil {
		t.Fatal(err)
	}
	defer s.Close()
